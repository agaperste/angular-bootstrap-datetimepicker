...
describe('DlDateTimePickerComponent startView=day', () => {
  ...
  describe('default behavior ', () => {
    ...
    it('should contain 1 .now element for the current day', () => {
    ...
    it('should NOT contain an .now element in the previous month', () => {
      // click on the left button to move to the previous hour
      debugElement.query(By.css('.left-button')).nativeElement.click();
      fixture.detectChanges();

      const currentElements = fixture.debugElement.queryAll(By.css('.now:not(.future):not(.past)'));
      expect(currentElements.length).toBe(0);
    });

    it('should NOT contain an .now element in the next month', () => {
      // click on the left button to move to the previous hour
