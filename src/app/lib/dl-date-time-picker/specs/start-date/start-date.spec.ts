...
describe('DlDateTimePickerComponent startDate', () => {
  ...
  describe('year', () => {
    ...
    it('should start on the startDate', () => {
      expect(debugElement.query(By.css('.view-label')).nativeElement.textContent.trim()).toBe('Oct 1985');
    });

    it('should change to the selected date after value is set', () => {
      component.picker.value = new Date(1995, APR, 18).getTime();
      fixture.detectChanges();
      expect(debugElement.query(By.css('.view-label')).nativeElement.textContent.trim()).toBe('Apr 1995');
    });
  });
  ...
  describe('undefined', () => {
    ...
    it('should start the current date', () => {
      expect(debugElement.query(By.css('.view-label')).nativeElement.textContent.trim()).toBe(moment().format('MMM YYYY'));
    });

    it('should change to the selected date after value is set', () => {
      component.picker.value = new Date(1966, MAY, 6).getTime();
      fixture.detectChanges();
      expect(debugElement.query(By.css('.view-label')).nativeElement.textContent.trim()).toBe('May 1966');
    });
